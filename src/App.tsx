...
import { BookOpen, CalendarDays, Check, Download, Flame, Headphones, Heart, LockKeyhole, Music2, NotebookPen, RotateCcw, Sparkles, Volume2, VolumeX, X } from 'lucide-react'
...
const formatDay = (day: number) => `Day ${String(day).padStart(2, '0')}`
  ...
  return <motion.div className="modal-backdrop" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={e=>e.target===e.currentTarget&&onClose()}>
    <motion.section className="reveal-card" initial={{scale:.86,y:45,opacity:0}} animate={{scale:1,y:0,opacity:1}} exit={{scale:.9,opacity:0}} transition={{type:'spring',damping:22}}>
      ...
      {!unwrapped ? <div className="wrapped-view">
        <p className="eyebrow">December {book.day} · waiting for you</p><button className="gift" onClick={()=>setUnwrapped(true)} aria-label={`Unwrap ${book.title}`}><div className="gift-paper">✦<span className="ribbon-v"/><span className="ribbon-h"/><i className="bow b1"/><i className="bow b2"/></div></button>
        ...
        <div className="reveal-info"><span className="little-star">✦</span><h2>{book.title}</h2><p className="author">by {book.author}</p><div className="rule"/><dl><div><dt>Genre</dt><dd>{book.genre}</dd></div><div><dt>Reading time</dt><dd>{book.length}</dd></div></dl><div className="tags">{book.mood.map(tag=><span key={tag}>{tag}</span>)}</div>
          <div className="actions"><a className="primary-action" href={book.file} target="_blank" rel="noreferrer" onClick={onOpened}><BookOpen size={17}/> Read online</a><a className="secondary-action" href={book.file} download onClick={onOpened}><Download size={16}/> {book.format}</a></div>
...
function CalendarView({ opened, onSelect }: { opened:number[]; onSelect:(b:Book)=>void }) {
  const availableDay = availableDayFor()
  return <motion.main className="page-view" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}}><div className="page-heading"><p className="eyebrow">Thirteen evenings together</p><h1>Your 13-day book advent</h1><p>Every night, another spine finds its place in the nook.</p></div><div className="calendar-grid">{booksData.map(book=><button key={book.day} className={`calendar-book ${book.day<=availableDay?'available':'locked'} ${opened.includes(book.day)?'complete':''}`} onClick={()=>book.day<=availableDay&&onSelect(book)}><BookCover book={book} small/><span className="calendar-day">{formatDay(book.day)}</span><strong>{book.title}</strong><small>{book.author}</small>{book.day>availableDay&&<LockKeyhole size={13}/>} {opened.includes(book.day)&&<Check size={14}/>}</button>)}</div></motion.main>
}
...
function App() {
  ...
  const availableDay = availableDayFor()
  const progress = Math.round(opened.length / totalDays * 100)
  const nextBook = booksData.find(book => book.day <= availableDay && !opened.includes(book.day)) ?? booksData[Math.min(availableDay, totalDays) - 1]
  const markOpened=()=>{if(selected&&!opened.includes(selected.day))setOpened([...opened,selected.day].sort((a,b)=>a-b))}
  const resetProgress=()=>{setOpened([]); setFinalLetter(false)}
  return <div className="app-shell">
    ...
    <header><button className="brand" onClick={()=>setView('room')}><span>☾</span><div><strong>The Midnight</strong><small>Reading Nook</small></div></button><nav><button className={view==='room'?'active':''} onClick={()=>setView('room')}><Flame/>The Nook</button><button className={view==='calendar'?'active':''} onClick={()=>setView('calendar')}><CalendarDays/>Calendar</button><button className={view==='journal'?'active':''} onClick={()=>setView('journal')}><NotebookPen/>Journal</button></nav><div className="header-tools"><button className="next-read" onClick={()=>setSelected(nextBook)}><Sparkles size={13}/> Next read</button><span className="progress-pill">{opened.length} <i>/ {totalDays} stories</i></span><button className="sound" onClick={()=>setSound(!sound)} aria-label="Toggle rain sounds">{sound?<Volume2/>:<VolumeX/>}</button><button className="sound reset" onClick={resetProgress} aria-label="Reset reading progress"><RotateCcw/></button></div><div className="top-progress" aria-hidden="true"><span style={{width:`${progress}%`}}/></div></header>
