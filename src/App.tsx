...
// Calendar now starts July 10, 2026 — Day 1 unlocks on the 10th, Day 2 on the 11th, and so on.
const calendarStart = new Date('2026-07-10T00:00:00')
...
function availableDayFor(date = new Date()) {
  ...
  // Days elapsed since calendarStart, +1 so the start date itself is Day 1.
  const elapsed = Math.floor((date.getTime() - calendarStart.getTime()) / msPerDay) + 1
  ...
  return <div className={`room stage-${stage}`}>
    ...
    {booksData.map((book, i) => {
      ...
      return <button key={book.day} className={`day-spot ${unlocked ? 'unlocked' : 'locked'} ${isOpened ? 'opened' : ''}`} style={{left:`${spots[i][0]}%`,top:`${spots[i][1]}%`}} onClick={()=>unlocked && onSelect(book)} aria-label={unlocked ? `Open day ${book.day}: ${book.title}` : `Day ${book.day}, locked`}>
        <span className="spot-number">{book.day}</span>{unlocked ? <Sparkles size={12}/> : <LockKeyhole size={10}/>}<span className="spot-tooltip">{unlocked ? (isOpened ? 'Visit again' : "Tonight's story") : `Opens Day ${book.day}`}</span>
      </button>
    })}
  </div>
}
  ...
  return <motion.div className="modal-backdrop" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={e=>e.target===e.currentTarget&&onClose()}>
    <motion.section className="reveal-card" initial={{scale:.86,y:45,opacity:0}} animate={{scale:1,y:0,opacity:1}} exit={{scale:.9,opacity:0}} transition={{type:'spring',damping:22}}>
      ...
      {!unwrapped ? <div className="wrapped-view">
        <p className="eyebrow">Day {book.day} · waiting for you</p><div className="gift" onClick={()=>setUnwrapped(true)}><div className="gift-paper">✦<span className="ribbon-v"/><span className="ribbon-h"/><i className="bow b1"/><i className="bow b2"/></div></div>
